    ...
    return withConnectivity(() async {
      try {
        ...
        final endpoint = isCurrentUser
            ? '${ApiRoutesHelper.journalistMePosts}?${Uri(queryParameters: queryParams).query}'
            : '${ApiRoutesHelper.journalistPosts(id)}?${Uri(queryParameters: queryParams).query}';
        final response = await _apiService.get(endpoint);
        dynamic responseData = response.data;
        if (responseData is Map && responseData['data'] != null) {
          responseData = responseData['data'];
  ...
  Future<List<Map<String, dynamic>>> getJournalists({
    ...
    return withConnectivity(() async {
      try {
        ...
        final endpoint =
            '${ApiRoutesHelper.journalists}?${Uri(queryParameters: queryParams).query}';
        final response = await _apiService.get(endpoint);
        final data = response.data['data'] ?? response;
