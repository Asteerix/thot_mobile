...
class AuthRepositoryImpl with ConnectivityAware {
  ...
  Future<UserProfile> getProfile() async {
    try {
      ...
      final response = await _apiService.get(ApiRoutes.authProfile);
      developer.log(
  ...
  Future<UserProfile> updateFormation({
    ...
    try {
      final response = await _apiService.put(
        ApiRoutes.updateFormation(formationId),
        data: {
  ...
  Future<UserProfile> deleteFormation(String formationId) async {
    try {
      final response = await _apiService.delete(
        ApiRoutes.deleteFormation(formationId),
      );
      if (response.data['success'] == false) {
        throw AuthException(
  ...
  Future<UserProfile> updateExperience({
    ...
    try {
      final response = await _apiService.put(
        ApiRoutes.updateExperience(experienceId),
        data: {
          if (title != null) 'title': title.trim(),
  ...
  Future<UserProfile> deleteExperience(String experienceId) async {
    try {
      final response = await _apiService.delete(
        ApiRoutes.deleteExperience(experienceId),
      );
      if (response.data['success'] == false) {
        throw AuthException(
  ...
  Future<void> logout() async {
    try {
      ...
      try {
        await _apiService.post(ApiRoutes.logout);
        developer.log(
