...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> logout() async {
    try {
      ...
      Future.microtask(() {
        final context = ServiceLocator.instance.navigatorKey.currentContext;
        if (context != null && context.mounted) {
          LoggerService.instance.info('Forcing navigation to /welcome after logout');
          try {
            GoRouter.of(context).go('/welcome');
          ...
          LoggerService.instance.warning('Context not available for navigation');
        }
      });
    } catch (e) {
      ...
      Future.microtask(() {
        final context = ServiceLocator.instance.navigatorKey.currentContext;
        if (context != null && context.mounted) {
          LoggerService.instance.info('Logout failed but navigating to /welcome anyway');
          try {
            GoRouter.of(context).go('/welcome');
            LoggerService.instance.info('Navigation to /welcome completed (error recovery)');
          } catch (navError) {
            LoggerService.instance.error('Navigation error during logout fallback', navError);
          }
        } else {
          LoggerService.instance.warning('Context not available for navigation (error recovery)');
        }
      });
  ...
  Future<void> deleteAccount() async {
    try {
      ...
      _userProfile = null;
      _isAdminMode = false;
      LoggerService.instance.info('Account deletion successful - state cleared');
      notifyListeners();
      Future.microtask(() {
        final context = ServiceLocator.instance.navigatorKey.currentContext;
        if (context != null && context.mounted) {
          LoggerService.instance.info('Forcing navigation to /welcome after account deletion');
          try {
            GoRouter.of(context).go('/welcome');
            LoggerService.instance.info('Navigation to /welcome completed');
          } catch (e) {
            LoggerService.instance.error('Navigation error after account deletion', e);
          }
        } else {
          LoggerService.instance.warning('Context not available for navigation');
        }
