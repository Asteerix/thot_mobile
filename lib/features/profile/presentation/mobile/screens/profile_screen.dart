...
import 'package:thot/shared/widgets/common/app_header.dart';
...
import 'package:thot/features/profile/presentation/shared/widgets/profile_header.dart';
...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  void _setupEventListeners() {
    _postCreatedSubscription =
      ...
      if (widget.isCurrentUser ||
        ...
        if (event.postType == PostTypes.question) {
          _logger.info('Calling _loadQuestions() from ProfileScreen');
          _loadQuestions();
        } else if (event.postType == PostTypes.short) {
  ...
  Future<void> _fetchProfileData() async {
    if (!mounted) return;
    _logger.info('📱 [ProfileScreen] Starting profile fetch - userId: ${widget.userId}, isCurrentUser: ${widget.isCurrentUser}');
    setState(() => _isLoading = true);
    try {
      if (widget.forceReload) {
        _logger.info('🔄 [ProfileScreen] Force reload requested - clearing cache');
        setState(() {
          _userProfile = null;
          _posts = null;
          _shorts = null;
      ...
      if (widget.isCurrentUser) {
        ...
        try {
          final response =
              await ServiceLocator.instance.apiService.get(ApiRoutes.authProfile);
          _logger.info('✅ [ProfileScreen] API response received - status: ${response.statusCode}');
          ...
          if (responseData != null && responseData is Map) {
            if (responseData['user'] != null && responseData['user'] is Map) {
              ...
              _logger.info('✅ [ProfileScreen] Profile data extracted from user key');
              ...
              if (responseData['data'] is Map) {
                if (responseData['data']['user'] != null &&
                  ...
                  _logger.info('✅ [ProfileScreen] Profile data extracted from data.user key');
                } else {
                  profileData = Map<String, dynamic>.from(responseData['data']);
                  _logger.info('✅ [ProfileScreen] Profile data extracted from data key');
                }
              } else {
                _logger.error('❌ [ProfileScreen] Invalid data type: ${responseData['data'].runtimeType}');
                throw Exception(
              ...
              _logger.info('✅ [ProfileScreen] Profile data extracted from root');
            }
          } else {
            _logger.error('❌ [ProfileScreen] Invalid response type: ${responseData.runtimeType}');
            throw Exception(
                'Invalid response format: expected Map but got ${responseData.runtimeType}');
          }
        } catch (e) {
          _logger.error('❌ [ProfileScreen] API call failed: $e - trying cache fallback');
          try {
            final user = await authRepository.getProfile();
            profileData = user.toJson();
            _logger.info('✅ [ProfileScreen] Profile loaded from cache');
          } catch (profileError) {
            _logger.error('❌ [ProfileScreen] Cache fallback also failed: $profileError');
            throw Exception('Failed to get current user: $profileError');
          }
        }
      } else {
        if (widget.userId == null) {
          _logger.warning('⚠️ [ProfileScreen] No userId provided for other user profile, treating as current user');
          final authRepository = ServiceLocator.instance.authRepository;
          try {
            final user = await authRepository.getProfile();
            profileData = user.toJson();
            _logger.info('✅ [ProfileScreen] Profile loaded as current user (fallback)');
          } catch (e) {
            _logger.error('❌ [ProfileScreen] Failed to load current user (fallback): $e');
            throw Exception('Failed to load profile: $e');
          }
        } else {
          _logger.info('👤 [ProfileScreen] Loading other user profile - userId: ${widget.userId}');
          try {
            final result =
                await _profileRepository.getUserProfile(widget.userId!);
            result.fold(
              (failure) {
                _logger.error('❌ [ProfileScreen] Failed to load other user profile: ${failure.message}');
                throw Exception('Failed to load profile: ${failure.message}');
              },
              (userData) {
                profileData = userData;
                _logger.info('✅ [ProfileScreen] Other user profile data loaded');
              },
            );
          } catch (e) {
            _logger.error('❌ [ProfileScreen] Exception loading other user profile: $e');
            throw Exception('Failed to load profile: $e');
          }
        }
      }
      ...
      _logger.info('✅ [ProfileScreen] UserProfile created - id: ${userProfile.id}, name: ${userProfile.name}, isJournalist: ${userProfile.isJournalist}');
      if (!mounted) {
        _logger.info('⚠️ [ProfileScreen] Widget unmounted after profile creation');
        return;
      }
      _updateTabController(userProfile.isJournalist);
      if (!mounted) {
        _logger.info('⚠️ [ProfileScreen] Widget unmounted after tab controller update');
        return;
      }
      setState(() {
      ...
      _logger.info('🔄 [ProfileScreen] Loading profile content (posts, shorts, questions)');
  ...
  Future<void> _loadShorts() async {
    ...
    if (_userProfile == null) {
      _logger.warning('⚠️ [ProfileScreen] User profile is null, cannot load shorts');
      setState(() => _shorts = []);
      return;
    }
    try {
      ...
      if (_userProfile!.isJournalist) {
        _logger.info('📰 [ProfileScreen] Loading shorts for journalist: ${_userProfile!.id}');
        final shortsResult = await _postRepository.getPosts(
          page: 1,
          userId: widget.userId ?? _userProfile!.id,
          type: PostTypes.short,
          status: 'published',
        );
        response = shortsResult['posts'] ?? [];
        _logger.info('✅ [ProfileScreen] Loaded ${response.length} shorts for journalist');
      } else {
        _logger.info('👤 [ProfileScreen] Loading public shorts for reader: ${_userProfile!.id}');
        try {
          final result = await _profileRepository.getUserPublicContent(
            widget.userId ?? _userProfile!.id,
            contentType: 'shorts',
          ...
          final publicContent = result.fold(
            (failure) {
              _logger.error('❌ [ProfileScreen] Failed to load public shorts: ${failure.message}');
              throw Exception(
                  'Failed to load public content: ${failure.message}');
            },
            (data) {
          ...
          _logger.info('✅ [ProfileScreen] Loaded ${response.length} public shorts');
        } catch (e) {
          _logger.error('❌ [ProfileScreen] Exception loading public shorts: $e');
          response = [];
        }
      }
      if (mounted) {
        setState(() {
          if (!(_userProfile?.isJournalist ?? true)) {
            _shorts = response;
            _logger.info('✅ [ProfileScreen] Set ${_shorts!.length} shorts (reader)');
          } else {
            _shorts = response
                .where((item) =>
                    item['type'] == PostTypes.short &&
                    item['metadata'] != null &&
                    item['metadata']['short'] != null)
                .toList();
            _logger.info('✅ [ProfileScreen] Set ${_shorts!.length} shorts (journalist, filtered)');
          }
        });
      } else {
        _logger.warning('⚠️ [ProfileScreen] Widget unmounted, not setting shorts state');
      }
    } catch (e, stackTrace) {
