...
import 'package:thot/features/admin/widgets/admin_dialog_utils.dart';
import 'package:thot/features/admin/widgets/admin_snackbar_utils.dart';
...
class AdminModerationActions extends StatelessWidget {
  ...
  Future<void> _banUser(BuildContext context) async {
    final reason = await AdminDialogUtils.showInputDialog(
      ...
      message:
          'Cette action est permanente. L\'utilisateur ne pourra plus accéder à son compte.',
      ...
      try {
        ...
        if (context.mounted) {
          AdminSnackbarUtils.showSuccess(
              context, 'Utilisateur banni avec succès');
        }
        onBanned?.call();
      } catch (e) {
  ...
  Future<void> _deleteContent(
    ...
    if (confirmed) {
      try {
        ...
        if (context.mounted) {
          AdminSnackbarUtils.showSuccess(
              context, 'Contenu supprimé avec succès');
        }
        onDeleted?.call();
      } catch (e) {
            ...
            children: [
              ...
              if (commentId != null)
                _buildActionButton(
                  ...
                  onPressed: () =>
                      _deleteContent(context, 'comment', commentId!),
                  color: AppColors.warning,
                ),
              if (shortId != null)
                _buildActionButton(
