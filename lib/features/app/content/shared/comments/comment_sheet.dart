...
class _CommentsBottomSheetState extends State<CommentsBottomSheet> {
      ...
      result.fold(
        ...
        (replies) {
          ...
          if (replies.isNotEmpty) {
            ...
            debugPrint(
                '   - Content: ${replies[0].content.substring(0, replies[0].content.length > 50 ? 50 : replies[0].content.length)}...');
          ...
          if (mounted) {
            setState(() {
              ...
              debugPrint(
                  '✅ [COMMENT_SHEET] State updated | repliesMap size: ${_repliesMap.length}, expandedComments size: ${_expandedComments.length}');
            });
          } else {
            debugPrint(
                '⚠️ [COMMENT_SHEET] Widget not mounted, skipping state update');
          }
        },
      );
    } catch (e, stackTrace) {
  ...
  Future<void> _sendComment() async {
    ...
    debugPrint(
        '✍️ [COMMENT_SHEET] Content: "${text.substring(0, text.length > 50 ? 50 : text.length)}${text.length > 50 ? "..." : ""}"');
    ...
    debugPrint(
        '✍️ [COMMENT_SHEET] Replying To Username: ${_replyingToUsername ?? "null"}');
    debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    try {
      ...
      result.fold(
        ...
        (newComment) {
          ...
          debugPrint(
              '✅ [COMMENT_SHEET] Parent Comment: ${newComment.parentComment ?? "null"}');
          debugPrint(
              '✅ [COMMENT_SHEET] IS REPLY: ${newComment.parentComment != null ? "✅ YES" : "❌ NO"}');
          debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
          if (!mounted) return;
          setState(() {
            if (newComment.parentComment != null) {
              ...
              debugPrint(
                  '✍️ [COMMENT_SHEET] Looking for parent comment: ${newComment.parentComment}');
              final parentIndex =
                  _comments.indexWhere((c) => c.id == newComment.parentComment);
              debugPrint(
                  '✍️ [COMMENT_SHEET] Parent comment index: $parentIndex');
              if (parentIndex != -1) {
                final parentComment = _comments[parentIndex];
                debugPrint('✍️ [COMMENT_SHEET] ✅ Parent comment found');
                debugPrint(
                    '✍️ [COMMENT_SHEET] Parent current reply count: ${parentComment.replyCount}');
                _comments[parentIndex] = parentComment.copyWith(
                  replyCount: parentComment.replyCount + 1,
                );
                debugPrint(
                    '✍️ [COMMENT_SHEET] Parent new reply count: ${_comments[parentIndex].replyCount}');
                if (_expandedComments.contains(newComment.parentComment)) {
                  debugPrint(
                      '✍️ [COMMENT_SHEET] ✅ Parent is expanded, adding reply to visible list');
                  if (_repliesMap[newComment.parentComment] != null) {
                    debugPrint(
                        '✍️ [COMMENT_SHEET] Adding to existing replies list (count: ${_repliesMap[newComment.parentComment]!.length})');
                  ...
                  debugPrint(
                      '✍️ [COMMENT_SHEET] New replies count: ${_repliesMap[newComment.parentComment]!.length}');
                } else {
                  debugPrint(
                      '✍️ [COMMENT_SHEET] ℹ️ Parent is collapsed, not showing reply in list');
                }
              } else {
                debugPrint(
                    '✍️ [COMMENT_SHEET] ⚠️ Parent comment not found in list!');
              }
            } else {
              debugPrint(
                  '✍️ [COMMENT_SHEET] 📝 Processing as TOP-LEVEL comment');
              _comments.insert(0, newComment);
              debugPrint('✍️ [COMMENT_SHEET] Added to top of comments list');
              debugPrint(
                  '✍️ [COMMENT_SHEET] Total comments: ${_comments.length}');
            }
            ...
            debugPrint(
                '✍️ [COMMENT_SHEET] ✅ State updated, reply context cleared');
          });
          SafeNavigation.showSnackBar(
            context,
            SnackBar(
          ...
          setState(() {
            ...
            if (index != -1) {
              ...
              for (final parentId in _repliesMap.keys) {
                final replies = _repliesMap[parentId]!;
                final replyIndex =
                    replies.indexWhere((r) => r.id == comment.id);
                if (replyIndex != -1) {
  ...
  Future<void> _deleteComment(Comment comment) async {
    ...
    debugPrint(
        '🗑️ [COMMENT_SHEET] Is Reply: ${comment.parentComment != null}');
    debugPrint(
        '🗑️ [COMMENT_SHEET] Parent ID: ${comment.parentComment ?? "null"}');
    debugPrint(
        '🗑️ [COMMENT_SHEET] Comments count before: ${_comments.length}');
    debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    try {
        ...
        (_) {
          ...
          setState(() {
            if (comment.parentComment != null) {
              ...
              debugPrint(
                  '✅ [COMMENT_SHEET] Removed reply from map | parentId: $parentId, before: $beforeCount, after: $afterCount');
              ...
              if (parentIndex != -1) {
                ...
                debugPrint(
                    '✅ [COMMENT_SHEET] Updated parent replyCount | oldCount: $oldCount, newCount: ${_comments[parentIndex].replyCount}');
              }
            } else {
              ...
              debugPrint(
                  '✅ [COMMENT_SHEET] Removed top-level comment | before: $beforeCount, after: $afterCount');
            }
          });
                  ...
                  : _comments.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final hasRepliesInMap =
                                _repliesMap[comment.id] != null;
                            final repliesCount =
                                _repliesMap[comment.id]?.length ?? 0;
                            debugPrint(
                                '🔍 [COMMENT_SHEET] Render check | commentId: ${comment.id}, isExpanded: $isExpanded, hasRepliesInMap: $hasRepliesInMap, repliesCount: $repliesCount');
                            return Column(
                          ...
                          Consumer<AuthProvider>(
                            builder: (context, authProvider, _) {
                              return UserAvatar(
                                ...
                                name: authProvider.userProfile?.name ?? authProvider.userProfile?.username,
                                isJournalist: authProvider.userProfile?.isJournalist ?? false,
                                radius: 16,
                              );
                            },
                          ),
...
class _InstagramCommentItemState extends State<_InstagramCommentItem>
  ...
  String _formatTimeAgo(DateTime dateTime) =>
      DateFormatter.formatTimeAgo(dateTime);
        ...
        child: Row(
          ...
          children: [
            UserAvatar(
              ...
              isJournalist: widget.comment.author.isJournalist,
              radius: 18,
            ),
            const SizedBox(width: 12),
            Expanded(
