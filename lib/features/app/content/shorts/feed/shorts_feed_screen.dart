...
class _ShortsFeedScreenState extends State<ShortsFeedScreen> {
  ...
  Future<void> _loadShorts() async {
    ...
    try {
      _logger.info(
          'Calling postService.getPosts with type: ${widget.isLiveMode ? PostType.live : PostType.short}');
      final response = await _postRepository.getPosts(
        type: widget.isLiveMode ? PostType.live.name : PostType.short.name,
      );
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (_cameras.length > 1)
                      IconButton(
                        icon: Icon(
                          _isFrontCamera ? Icons.camera_alt : Icons.camera_alt,
                          color: Colors.white,
                        ),
